    ...
    public void onCreatePreferences(Bundle bundle, String s) {
        ...
        for (int i=0; i < count; i++){
            ...
            if (!(p instanceof CheckBoxPreference)){
                String entryValue = sharedPreferences.getString(p.getKey(),"");
                setPreferenceSummary(p,entryValue);
            }
        }
    }

    // done (2) Create a setPreferenceSummary which takes a Preference and String entryValue as parameters.
    // This method should check if the preference is a ListPreference and, if so, find the label
    // associated with the entryValue. You can do this by using the findIndexOfValue and getEntries methods
    // of Preference.
